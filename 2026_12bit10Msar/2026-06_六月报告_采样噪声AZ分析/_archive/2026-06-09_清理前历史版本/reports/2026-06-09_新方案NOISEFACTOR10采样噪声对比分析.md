> 命名更正：这组数据是“对比方案”，不是新方案。正式报告请看 `2026-06-09_对比方案NOISEFACTOR10采样噪声对比分析.md`。

# 2026-06-09 新方案：NOISEFACTOR=10 仅采样噪声结果对比分析

## 数据来源

- 当前新方案原始数据：`raw_data/2026-06-09_sampling_only_NOISEFACTOR10_rerun/ExplorerRun.0.rdb`
- 当前新方案日志：`raw_data/2026-06-09_sampling_only_NOISEFACTOR10_rerun/ExplorerRun.0.log`
...
下面是同样 `td=28n/AZ≈4ns` 条件下，新方案相对昨天结果的变化：

| corner | SNR 昨天 | SNR 新方案 | ΔSNR | ENOB 昨天 | ENOB 新方案 | ΔENOB |
| --- | --- | --- | --- | --- | --- | --- |
...
- 新方案 `td=28n`：平均 ENOB `10.3476`，平均 SNR `64.2697 dB`，平均 SFDR `75.9646 dB`。
...
按逐 corner residual 简单平均，新方案 `td=28n` 的采样噪声功率剩余约 `11.82%`，消除约 `88.18%`；昨天 `td=28n` 用同一口径对应平均剩余约 `11.53%`，消除约 `88.47%`。如果按平均 SNR 直接折算，新方案剩余约 `10.16%`，消除约 `89.84%`；昨天报告里约 `10.92%` 剩余、`89.08%` 消除采用的就是这个平均 SNR 口径。

结论是：平均意义上，新方案和昨天 `td=28n` 很接近，甚至平均 SNR 略好；但它的 corner 分布更不均匀，`ss` corner 是主要风险。
...
平均上，新方案 `NOISEFACTOR=10` 采样噪声压力测试比全噪声标称 `td=28n` 低 `-7.0199 dB` SNR、低 `-1.1394 bit` ENOB。这和昨天压力测试的判断一致：`NOISEFACTOR=10` 会把采样噪声人为推成主导项，不能直接代表正常全噪声规格，但很适合看 AZ 对采样噪声本身的处理能力。

## 基本结论

1. 新方案这轮 `td=28n/AZ≈4ns` 的平均 ENOB/SNR 与昨天 `td=28n` 基本一致：平均 ENOB 几乎不变，平均 SNR 略高约 `0.3123 dB`。
2. 新方案的 corner 分布更偏：`ff/fs` 改善，`ss/tt` 变差，其中 `ss` 是 worst-case，SNR 只有 `61.28 dB`。
3. 以昨天 `td=24.05n` 作为基准折算，新方案采样噪声功率逐 corner 平均剩余 `11.82%`，约消除 `88.18%`；与昨天 `4ns` 的 `11.53%` 剩余相比，平均水平非常接近。
4. 和刚刚全噪声标称相比，新方案仍低约 `7.02 dB` SNR，说明 `NOISEFACTOR=10` 采样噪声压力测试仍是主导噪声验证，不应直接当正常全噪声规格。

## 输出文件

- `tables/sampling_nf10_new_scheme_metrics.csv`
- `tables/sampling_nf10_new_scheme_aggregate.csv`
- `tables/sampling_nf10_new_vs_yesterday_td28_comparison.csv`
- `tables/sampling_nf10_new_scheme_cancellation_using_yesterday_baseline.csv`
- `tables/sampling_nf10_new_vs_full_noise_td28_comparison.csv`
- `figures/2026-06-09_sampling_only_NOISEFACTOR10_rerun/new_scheme_vs_yesterday_snr_by_corner.png`
- `figures/2026-06-09_sampling_only_NOISEFACTOR10_rerun/td28_three_scheme_snr_by_corner.png`
- `figures/2026-06-09_sampling_only_NOISEFACTOR10_rerun/new_scheme_residual_noise_by_corner.png`
