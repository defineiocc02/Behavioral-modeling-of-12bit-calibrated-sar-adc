...
STAGE_SPECS: tuple[StageSpec, ...] = tuple([
    ...
    StageSpec(index=13, name="TERMINAL", nominal_weight=1,  is_physical=False, role="rounding"),
])
...
# calDAC 搜索阶段: w7..w12 (L32C 降序到 L1C)
CAL_DAC_STAGES = [7, 8, 9, 10, 11, 12]  # L32C, L16C, L8C, L4C, L2C, L1C
CAL_DAC_SEARCH_COUNT = 6                 # centered_residual_mode 只有 6 步
...
# 噪声参数 (对齐策略文档 §3 P1-S4)
# ============================================================================
CAL_NOISE_SIGMA = 0.50      # 校准期间比较器噪声 RMS (LSB), 最优抖动
CAL_OFFSET_SIGMA = 0.5      # 失调标准差 (LSB)
CAL_ZERO_TIE_THRESHOLD = 0.2  # 零残差检测阈值 (LSB), 对齐 VA ~0.003*VFS
CONV_NOISE_SIGMA = 0.15     # 正常转换期间比较器噪声 RMS (LSB)
...
# MC 参数
MC_SIGMA_C_REL = 0.02       # 单位电容相对标准差 2%
